...
/**
 * Продолжает ли новое письмо уже начатую переписку.
 *
 * Ответ намеренно бедный: номер дела и объяснение. Ничего переписывать модель
 * здесь не может — только сказать, к какому из показанных дел письмо
 * относится, либо что ни к какому.
 */
export const CONTINUATION_SCHEMA: JSONSchema = object({
  continues_case_id: {
    type: "integer",
    description:
      "Номер дела из показанного списка, о котором идёт речь в новом письме. " +
      "0 — если это новая тема и ни одно из дел не подходит",
  },
  why: str(
    "Одна фраза: что именно в письме указывает на это дело — встреча, дата, " +
      "договорённость. Для 0 — почему тема новая",
  ),
});
...
export const REPLY_ACTIONS = [
  ...
  "escalate",
  "close",
  /** Вопрос решён или клиника отказалась — короткое спасибо и прощание. */
  "farewell",
  /** Переписка уже завершена прощанием, новое письмо ответа не требует. */
  "silent",
] as const;
...
export const REPLY_DECISION_SCHEMA: JSONSchema = object({
  action: {
    ...
    description:
      ...
      "escalate — красный флаг §6, передать человеку; close — клиника запретила ей писать; " +
      "farewell — вопрос закрыт (встреча подтверждена и вопросов не осталось) либо клиника " +
      "отказалась от услуги: короткое письмо с благодарностью и прощанием; " +
      "silent — мы уже попрощались, а новое письмо ответа не требует",
  },
  subject: str("Тема ответа, с префиксом Re: если это ответ на письмо"),
  body: str(
    "Законченный текст письма на русском по шаблонам регламента, 4–8 строк, готовый к отправке. " +
      "Для action=farewell — 2–4 строки: поблагодарить и попрощаться, ничего нового не спрашивая. " +
      "Для action=escalate, close и silent оставь пустым: такие письма человек пишет сам " +
      "либо не пишет никто",
  ),
  booking: object({
...
export const TRIAGE_SCHEMA: JSONSchema = object({
  spam_roots: arr(
    str("root_message_id цепочки, которая является явным мусором"),
    "Только явный мусор: рекламные рассылки, соцсети, игры, доставка еды, промо банков, " +
      "уведомления сервисов, подтверждения заказов. Всё остальное — включая любое упоминание " +
      "здоровья, медицины, препаратов и любую деловую переписку — сюда НЕ попадает",
  ),
});
