...
/**
 * Решение по входящему письму от клиники — по регламенту §4.
 *
 * Модель не просто пишет текст, а объявляет действие и подкрепляет данные
 * дословными цитатами: §1 требует, чтобы каждый пункт брони опирался на
 * слова из переписки. Код потом проверяет цитаты на подлинность и сверяет
 * дату с календарём — выдуманный адрес или несуществующий четверг до
 * отправки не доходят.
 */
export const REPLY_DECISION_SCHEMA: JSONSchema = object({
  action: {
    type: "string",
    enum: [...REPLY_ACTIONS],
    description:
      "book — все данные §1 есть и можно бронировать; clarify — не хватает 1–3 пунктов; " +
      "contradiction — переписка себе противоречит; alternatives — время занято или вне окна; " +
      "escalate — красный флаг §6, передать человеку; close — клиника отказалась от переписки",
  },
  subject: str("Тема ответа, с префиксом Re: если это ответ на письмо"),
  body: str(
    "Законченный текст письма на русском по шаблонам регламента, 4–8 строк, готовый к отправке. " +
      "Для action=escalate оставь пустым: такие письма человек пишет сам",
  ),
  booking: object({
    date: str("Дата встречи ГГГГ-ММ-ДД или пустая строка, если не названа"),
    time: str("Время ЧЧ:ММ или пустая строка, если не названо"),
    weekday_in_letter: str("День недели, названный клиникой, или пустая строка"),
    duration_minutes: num("Длительность в минутах; 0, если не названа — тогда берётся дефолт §5"),
    clinic_name: str("Название клиники или пустая строка"),
    contact: str("Контактное лицо: имя и должность, или пустая строка"),
    format: str("на месте | онлайн | звонок, или пустая строка"),
    location: str("Полный адрес с этажом и кабинетом, платформа или чей номер набирают"),
    topic: str("Тема встречи одной строкой"),
    consent_quote: str("Дословная цитата с согласием клиники (§2) или пустая строка"),
    authority_quote: str("Дословная цитата, показывающая должность отправителя, или пустая строка"),
  }, "Данные брони по §1. Пустая строка там, где в переписке этого нет — не додумывай"),
  quotes: arr(
    str("Дословная цитата из переписки, подтверждающая один из пунктов §1"),
    "Цитаты, на которых держится решение. Копировать буквально, иначе пункт не засчитается",
  ),
  red_flags: arr(
    str("Красный флаг §6, найденный в письме"),
    "Цена, договор, юридический вопрос, ПДн пациента, жалоба, медицинский вопрос по существу, " +
      "поздняя отмена, смена контакта или домена. Пустой массив, если ничего нет",
  ),
  missing: arr(
    str("Какого пункта §1 не хватает"),
    "Незаполненные обязательные пункты. Пустой массив, если есть всё",
  ),
  clarifications: arr(
    CLARIFICATION,
    "Чего не хватило о нашей стороне — вопросы пользователю, не клинике. Пустой массив, если всё есть",
  ),
});
