...
/**
 * День недели в НАШЕМ письме против календаря.
 *
 * §3 требует, чтобы день недели сходился с датой, и код проверяет это в
 * письме клиники. Но тот же день недели пишет и модель — в подтверждении,
 * которое уходит наружу: «03.09.2026, среда, 18:00», когда 3 сентября —
 * четверг. Клиника приедет в среду, и виновата будет не она.
 *
 * Слово в нашем письме — не факт из переписки, а вычислимая величина, и
 * поправить её можно молча. Правим, только когда день недели в тексте один:
 * если их несколько, письмо устроено сложнее шаблона, и слепая замена
 * исказит смысл — тогда просто сообщаем о расхождении.
 */
const WEEKDAY_FORMS: Array<[string, RegExp]> = [
  ["воскресенье", /воскресень[ея]/giu],
  ["понедельник", /понедельник[аи]?/giu],
  ["вторник", /вторник[аи]?/giu],
  ["среда", /сред[ауы]/giu],
  ["четверг", /четверг[аи]?/giu],
  ["пятница", /пятниц[ауы]/giu],
  ["суббота", /суббот[ауы]/giu],
];

export function fixWeekdayIn(
  text: string,
  date: Date,
): { text: string; wrong: string | null; fixed: boolean } {
  const actual = weekdayOf(date);
  const found = WEEKDAY_FORMS.filter(([, re]) => new RegExp(re.source, "iu").test(text));

  const wrong = found.find(([name]) => name !== actual);
  if (!wrong) return { text, wrong: null, fixed: false };

  // Несколько разных дней недели в письме — не шаблон подтверждения, не трогаем.
  if (found.length > 1) return { text, wrong: wrong[0], fixed: false };

  return { text: text.replace(wrong[1], actual), wrong: wrong[0], fixed: true };
}
...
const ESCALATE_PATTERNS: Array<{ pattern: RegExp; label: string }> = [
  ...
  // §6 «обязательства, которых мы не давали»: клиника ссылается на обещание
  // или требует гарантию. Решать такое за компанию агент не вправе.
  // `ответственност`, а не `ответственн`: иначе «ответственный менеджер» в
  // подписи каждого второго письма уводило бы переписку человеку.
  {
    pattern: flag("гаранти|обязательств|обязуетес|ответственност|штраф"),
    label: "обязательства и гарантии",
  },
];
...
/**
 * Адрес, на который отвечать бессмысленно: за ним никого нет.
 *
 * Проверка детерминированная и стоит раньше модели — намеренно. Ответ на
 * `noreply@` не доходит ни до кого: в лучшем случае он молча пропадёт, в
 * худшем вернётся отлупом, который снова ляжет в ящик как входящее письмо,
 * заведёт дело и получит ответ. Так уже было: агент переписывался с
 * `postmaster@`, отвечая на его же сообщение о недоставке.
 *
 * Фильтруется только ОТПРАВКА. Приём — нет: клиники рассылают результаты
 * анализов и счета именно с `noreply@`, и такие письма нужны в делах.
 */
export function isUnreplyable(address: string): boolean {
  const local = address.toLowerCase().split("@")[0] ?? "";
  return (
    // Метку ищем в любом месте, а не только в начале. Привязка к началу
    // строки пропускала `families-noreply@google.com` и
    // `googleone-noreply@google.com` — Google ставит перед меткой название
    // сервиса. На первый из них агент письмо и отправил.
    /(?:^|[._-])(?:no[._-]?reply|do[._-]?not[._-]?reply|donotreply|noreply)/.test(local) ||
    /^(?:mailer-daemon|postmaster|bounce[sd]?)$/.test(local)
  );
}
...
/**
 * §4: слова, которыми переписку заканчивают, — отказ от услуги или прощание.
 *
 * Это не запрет писать (`looksLikeRefusal`), а конец разговора: «нам это не
 * нужно», «мы не хотим продолжать», «всего доброго». На такое письмо агент
 * отвечает последний раз — благодарностью и прощанием, — и закрывает дело.
 * Пропущенный признак стоит одного лишнего уточняющего письма, поэтому слова
 * подобраны узко: обычное «спасибо» в середине переписки концом не считается.
 */
export function looksLikeGoodbye(text: string): boolean {
  return /не нужн|не интересн|не хотим продолж|не будем продолж|отказыва|откажемся|всего доброго|до свидания|до связи|good ?bye|not interested|do ?n['’`]?t want to continue|no longer interested|nothing further/iu.test(
    text,
  );
}
