...
const RED_FLAG_PATTERNS: Array<{ pattern: RegExp; label: string }> = [
  ...
  { pattern: flag("диагноз|симптом|лечени|назначени|препарат|анализ|обследовани"), label: "медицинский вопрос по существу" },
  { pattern: flag("иин|паспорт|полис|дата рождения|пациент[аеу]?\\s+(?:зовут|фамили)"), label: "персональные данные пациента" },
];

/**
 * Красные флаги в тексте письма.
 *
 * Дублирует суждение модели намеренно: пропущенный флаг означает, что агент
 * сам ответит про цену, диагноз или претензию — ровно то, что регламент
